...
struct _game {
  ...
  BULLET
  *orphan_bullets[MAX_ORP_BULLETS]; ///< Array of bullets without an owner
  OBJECT *objects[MAX_OBJECTS];     ///< Array of other objects in the game
  OBJECT *bunkers[NUM_BUNKERS];     ///< Array of bunkers
...
STATUS game_destroy_martian(GAME *game,
                            int i); /**< Destroys specified martian */
STATUS game_colisions(GAME *game);  /**< Checks for collisions */

STATUS game_ship_update(
    GAME *game,
    ALLEGRO_KEYBOARD_STATE *key); /**< Updates ship based on input */
STATUS game_martians_update(GAME *game,
                            float speed);  /**< Updates all martians */
STATUS game_animations_update(GAME *game); /**< Updates animations */

STATUS game_add_orphan_bullet(GAME *game,
                              BULLET *bullet); /**< Adds an orphan bullet */
BULLET *
game_dequeue_orphan_bullet(GAME *game); /**< Dequeues an orphan bullet */
BULLET *
game_extract_orphan_bullet_at(GAME *game,
                              int i); /**< Extracts a specific orphan bullet */
BULLET *game_get_orphan_bullet_at(GAME *game,
                                  int i); /**< Gets a specific orphan bullet */
STATUS game_move_orphan_bullets(GAME *game,
                                float speed); /**< Moves orphan bullets */

STATUS game_add_object(GAME *game,
                       OBJECT *object);  /**< Adds an object to the game */
OBJECT *game_dequeue_object(GAME *game); /**< Removes an object from the game */
OBJECT *game_extract_object_at(GAME *game,
                               int i); /**< Extracts a specific object */
...
STATUS
game_print_other_elements(GAME *game); /**< Renders other game elements */
...
STATUS game_init(GAME *game, float FPS) {
  ...
  game->timer = al_create_timer(1.0 / game->FPS); // Creacion del tiempo del
                                                  // game
  ...
  game->screen =
      al_create_display(DISPLAY_WIDTH, DISPLAY_HEIGHT); // Creacion de pantalla
  if (!game->screen) {
    return ERROR;
  }
  ...
  al_convert_mask_to_alpha(
      game->frame,
      al_map_rgb(255, 0,
                 255)); // Que el color magenta sea tratado como transparente
  ...
  al_convert_mask_to_alpha(game->martian_explosion_img,
                           al_map_rgb(255, 0, 255));

  game->bunker_img = al_load_bitmap("resources/images/escudos.bmp");
  if (!game->bunker_img) {
    return ERROR;
  }
  ...
  game->font =
      al_load_ttf_font(FONT_RSC, 18, 0); // 24 es el tamaño de la fuente
  if (!game->font) {
    return ERROR;
  }

  game->event_queue =
      al_create_event_queue(); // Creacion de pila de eventos del game
  if (!game->event_queue) {
    return ERROR;
  }

  game->ship =
      ship_create(game->ship_img, SHIP_WIDTH, SHIP_HEIGHT, SHIP_INIT_POS_X,
  ...
  // Inicializacion de escudos
  float bunker_spacing = (DISPLAY_WIDTH - 80) / NUM_BUNKERS;
  for (int i = 0; i < NUM_BUNKERS; i++) {
    game->bunkers[i] =
        obj_create(game->bunker_img, 0, 0, BUNKER_WIDTH, BUNKER_HEIGHT,
                   60 + i * bunker_spacing, BUNKER_INIT_POS_Y, true);
    if (!game->bunkers[i]) {
      return ERROR;
    }
    obj_set_life(game->bunkers[i], BUNKER_LIFE);
  }
 ...
 */
STATUS game_destroy_martian(GAME *game,
                            int i) { // Cambiar estructura y devolver martian
...
STATUS game_ship_update(GAME *game, ALLEGRO_KEYBOARD_STATE *key) {
  ...
  if (al_key_down(key, ALLEGRO_KEY_SPACE)) {
    if (ship_shoot(game->ship, game->ship_bullet_img) == OK) {
      al_play_sample(game->samples[0], 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE,
                     NULL);
    }
  }
...
STATUS game_martians_update(GAME *game, float speed) {
  ...
  if (game->num_enemies_alive > 0) {
    ...
    if (++game->enemy_move_timer >= MART_MOVE_TIMER) {
      ...
      al_play_sample(game->samples[5 + move_sound_idx], 1.0, 0.0, 1.0,
...
STATUS game_colisions(GAME *game) {
  ...
  for (int i = ship_get_num_shots(game->ship) - 1; i >= 0; i--) {
    ...
    for (int j = 0; j < game->num_enemies_alive; j++) {
      ...
      if (bullet_check_colision(sb, mart_get_x(m), mart_get_y(m), MART_WIDTH,
                                MART_HEIGHT)) {
        al_play_sample(game->samples[1], 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE,
                       NULL);
        ...
        // Explosion
        OBJECT *exp =
            obj_create(game->martian_explosion_img, 0, 0, EXPLOSION_WIDTH,
                       EXPLOSION_HEIGHT, mart_get_x(m), mart_get_y(m), true);
        if (exp) {
    ...
    // Contra UFO
    if (game->ufo &&
        bullet_check_colision(sb, obj_get_x(game->ufo), obj_get_y(game->ufo),
                              UFO_WIDTH, UFO_HEIGHT)) {
      game->total_score +=
          UFO_POINTS_MIN + (rand() % (UFO_POINTS_MAX - UFO_POINTS_MIN));
      al_stop_samples();
      al_play_sample(game->samples[4], 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE,
                     NULL);
      // Explosion UFO
      OBJECT *exp = obj_create(
          game->martian_explosion_img, 0, 0, EXPLOSION_WIDTH, EXPLOSION_HEIGHT,
          obj_get_x(game->ufo), obj_get_y(game->ufo), true);
      if (exp) {
        obj_set_timer(exp, OBJECT_TIMER);
        game_add_object(game, exp);
      }
    ...
    for (int j = 0; j < NUM_BUNKERS; j++) {
      ...
      if (bullet_check_colision(sb, obj_get_x(game->bunkers[j]),
                                obj_get_y(game->bunkers[j]), BUNKER_WIDTH,
                                BUNKER_HEIGHT)) {
        int life = obj_get_life(game->bunkers[j]) - 1;
        obj_set_life(game->bunkers[j], life);
        obj_set_source_x(game->bunkers[j], (BUNKER_LIFE - life) / 3);
        bullet_destroy(ship_extract_bullet_at(game->ship, i));
        if (life <= 0) {
  ...
  for (int j = 0; j < game->num_enemies_alive; j++) {
    ...
    for (int k = mart_get_num_shots(m) - 1; k >= 0; k--) {
      ...
      if (GOD_MODE == 0 && !game->ship_exploding &&
          bullet_check_colision(
              mb, ship_get_x(game->ship), ship_get_y(game->ship),
              ship_get_width(game->ship), ship_get_height(game->ship))) {
        al_play_sample(game->samples[2], 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE,
                       NULL);
        // Iniciar animacion de explosion de la propia nave (paralizando el
        // juego)
  ...
  for (int i = game->num_orphan_bullets - 1; i >= 0; i--) {
    ...
    if (GOD_MODE == 0 && !game->ship_exploding &&
        bullet_check_colision(
            ob, ship_get_x(game->ship), ship_get_y(game->ship),
            ship_get_width(game->ship), ship_get_height(game->ship))) {
      al_play_sample(game->samples[2], 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE,
                     NULL);
...
STATUS game_animations_update(GAME *game) {
  ...
  if (++game->enemy_animation_timer >
      MART_ANIMATION_SPEED) { // temporizador animaciones imagen enemy
    for (int i = 0; i < game->num_enemies_alive;
         i++) { // movimiento animacion enemigo
      if (mart_get_source_x(game->enemy[i]) > 0) {
        mart_set_source_x(game->enemy[i], 0);
      } else {
        mart_set_source_x(
            game->enemy[i],
            1); // La segunda imagen del tile para simular el movimiento
      }

      if (mart_get_num_shots(game->enemy[i]) >
          0) { // movimiento animacion bala enemigo
    ...
    for (int i = 0; i < game->num_orphan_bullets;
         i++) { // movimiento animacion bala huerfana enemigo
      ...
      if (bullet_get_source_x(bullet) > 0) {
        ...
        bullet_set_source_x(
            bullet, 1); // La segunda imagen del tile para simular el movimiento
      }
    }
...
STATUS game_update(GAME *game, ALLEGRO_KEYBOARD_STATE *key) {
  ...
  current_mart_speed =
      MART_BASE_SPEED +
      (MART_MAX_SPEED - MART_BASE_SPEED) *
  ...
  case (ALLEGRO_EVENT_TIMER):
    ...
    if (game->ufo == NULL) {
      if (rand() % UFO_SPAWN_CHANCE == 0) {
        ...
        game->ufo_dir = -side;
        float sx =
            (side == -1 ? (float)FRAME_WIDTH - UFO_WIDTH : (float)CANVAS_WIDTH);
        game->ufo = obj_create(game->martian_img, 0, 0, UFO_WIDTH, UFO_HEIGHT,
                               sx, 143, false);
        if (game->ufo) {
          al_play_sample(game->samples[3], 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_LOOP,
                         NULL);
        }
      }
    } else {
      float ux = obj_get_x(game->ufo);
      if (ux < (float)FRAME_WIDTH - UFO_WIDTH - 5 ||
          ux > (float)CANVAS_WIDTH + 5) {
        al_stop_samples();
        obj_destroy(game->ufo);
        game->ufo = NULL;
      } else {
...
STATUS game_print_ship(GAME *game) {
  ...
  for (int i = 0; i < ship_get_num_shots(game->ship); i++) { // pintar bala nave
    bullet_print(ship_get_bullet_at(game->ship,
                                    i)); // quitar los sources de los parametros
  }
...
STATUS game_print_enemies(GAME *game) {
  ...
  for (int i = 0; i < game->num_enemies_alive; i++) { // pintar enemy
    ...
    for (int j = 0; j < mart_get_num_shots(game->enemy[i]);
         j++) { // pintar bala marciano
      BULLET *bullet = mart_get_bullet_at(
          game->enemy[i], j); // Se puede acceder sin una funcion extra
      bullet_print(bullet);
    }
  }
...
STATUS game_print_score(GAME *game) {
  ...
  // pintar puntuacion
  al_draw_text(game->font, al_map_rgb(255, 255, 255), FRAME_WIDTH + 10, 120, 0,
               "SCORE");
  sprintf(score_text, "%d", game->total_score);
  al_draw_text(game->font, al_map_rgb(0, 255, 0), FRAME_WIDTH + 90, 120, 0,
               score_text);
...
STATUS game_print_life(GAME *game) {
  ...
  // pintar vida
  al_draw_text(game->font, al_map_rgb(255, 255, 255), CANVAS_WIDTH - 180, 120,
               0, "LIVES");
  for (int i = 0, space_btw = 0; i < ship_get_life(game->ship);
       i++, space_btw += (ship_get_width(game->ship))) {
    al_draw_bitmap_region(
        ship_get_bitmap(game->ship),
